...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      {/* Mobile / Tablet Horizontal Navigation Bar (Visible only on screens < lg) */}
      <div className="lg:hidden w-full bg-white border-b border-slate-200/80 px-3 py-2.5 overflow-x-auto max-w-full flex items-center gap-2 shadow-xs text-xs sticky top-16 z-20">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl font-bold whitespace-nowrap transition-all shrink-0 active:scale-95 ${
                isActive
            ...
            >
              ...
              <span>{item.label.split('&')[0].split('(')[0].trim()}</span>
              {item.badge !== null && (
      ...
      <aside className="w-72 shrink-0 hidden lg:flex bg-white border-r border-slate-200/80 min-h-[calc(100vh-4rem)] p-4 flex-col justify-between">
